...
export const RequestList = () => {
  ...
  const deleteRequest = async (from, to) => {
    try {
      await axiosPrivate.delete(`/request?from=${from}&to=${to}`);
    } catch (err) {
      console.error(err);
      navigate("/login", { state: { from: location }, replace: true });
    }
  ...
  useEffect(() => {
    ...
    if (sender) {
      if (sender.status === 0) {
        deleteRequest(sender.sender, auth.username).then(
          setReceivedRequests(
  ...
  useEffect(() => {
    if (recipient) {
      deleteRequest(auth.username, recipient).then(
                    ...
                    <div key={index} className="card mb-3">
                      ...
                      <div className="card-body">
                        ...
                        <p className="card-text col-10">
                          ...
                          {request.text ? request.text : "No message."}
                        </p>
